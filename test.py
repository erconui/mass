from IPython.core.debugger import set_trace

import torch
import torch.nn as nn
import pdb
import torch.optim as optim
from torch.utils.data import DataLoader
import numpy as np

from tqdm import tqdm
from importlib import reload

import simulator
reload(simulator)
from simulator import Simulator
import rl_net
reload(rl_net)
from rl_net import PolicyNetwork, ValueNetwork, AdvantageDataset, PolicyDataset, calculate_returns, calculate_advantages
import pdb, traceback, sys


nodes = [
    (0.,0.), (100.,0.), (200.,0),#2
    (0.,100), (100.,100.), (200.,100.),#5
    (0.,200), (100.,200.), (200.,200.),#8
    (0.,300), (100.,300.), (200.,300.),#11
    (300.,0), (300,100), (300, 200), (300, 300),#15
    (0, 400), (100, 400), (200, 400), (300, 400),#19
    (400, 0), (400, 100), (400, 200), (400, 300), (400, 400)#24
    ]
#
edges = [(0, 1), (0, 3),
         (1, 2), (1, 4),
         (2, 5), (2, 12),
         (3, 4), (3, 6),
         (4, 5), (4, 7),
         (5, 8), (5, 13),
         (6, 7), (6, 9),
         (7, 10), (7, 8),
         (8, 14), (8, 11),
         (9, 10), (9, 16),
         (10, 11), (10, 17),
         (11, 15), (11, 18),
         (12, 13), (12, 20),
         (13, 14), (13, 21),
         (14, 15), (14, 22),
         (15, 19), (15, 23),
         (16, 17),
         (17, 18),
         (18, 19),
         (19, 24),
         (20, 21),
         (21, 22),
         (22, 23),
         (23, 24),
         ]
#
num_targets = 2
num_agents = 2
env = Simulator(nodes, edges, N=250, num_targets=num_targets, num_agents=num_agents)
states = env.get_state_dimensions()
actions = env.get_action_dimensions()
print(states, actions)
# set_trace()
if True:
    policy = PolicyNetwork(states, actions, num_agents)
    value = ValueNetwork(states)
else:
    policy = torch.load("policy-90")
    value = torch.load("value-90")
#
policy_optim = optim.Adam(policy.parameters(), lr=1e-6, weight_decay=0.01)
value_optim = optim.Adam(value.parameters(), lr=1e-6, weight_decay=1)
epochs = 901

value_criteria = nn.MSELoss()

# Hyperparameters
env_samples = 50
episode_length = 200
gamma = 0.9
value_epochs = 2
policy_epochs = 10
value_batch_size = 32
policy_batch_size = 256
epsilon = 0.2
# standing_time_list = []
max_x_list = []
loss_list = []
reward_list = []

# loop = tqdm(total=epochs, position=0, leave=False)

for epoch in range(epochs):
    # generate rollouts
    rollouts = []
    total_reward = 0
    sim_length = 0
    for _ in range(env_samples):
        # don't forget to reset the environment at the beginning of each episode!
        # rollout for a certain number of steps!
        current_rollout = []
        state = env.reset()
        reward = 0
        end_roll = False
        current_sim_length = 0
        for ii in range(episode_length):
            policy_vector = policy(torch.from_numpy(state).float().view(1,-1))
            # print(policy_vector.shape)
            action_prob = policy_vector.view(num_agents, -1)
            # print(action_prob.shape)
            action_index_list = [
                np.random.choice(range(action.shape[-1]),
                                 p=action.detach().numpy().reshape((action.shape[-1])))
                for action in action_prob]
            action_index = 0
            for index in reversed(action_index_list):
                action_index *= 24
                action_index += index

            s_prime, rr, end_roll = env.step(action_index_list)

            # action_index = torch.from_numpy(action_index)
            # print(state.shape, rr)

            reward += rr
            current_sim_length += 1

            current_rollout.append((state, policy_vector.detach().reshape(actions), action_index, rr))
            if end_roll:
                break
            #
            state = s_prime
        #
        sim_length += current_sim_length
        rollouts.append(current_rollout)
        total_reward += reward
    #
    print('now train')
    avg_sim_length = sim_length / env_samples
    avg_reward = total_reward / env_samples

    reward_list.append(avg_reward)

    calculate_returns(rollouts, gamma)

    # Approximate the value function
    value_dataset = AdvantageDataset(rollouts)
    value_loader = DataLoader(value_dataset, batch_size=value_batch_size, shuffle=True, pin_memory=True, drop_last=True)
    for _ in range(value_epochs):
        # train value network
        total_loss = 0
        # set_trace()
        try:
            for state, returns in value_loader:
                value_optim.zero_grad()
                returns = returns.unsqueeze(1).float()
                expected_returns = value(state.float())
                loss = value_criteria(expected_returns, returns)
                total_loss += loss.item()
                loss.backward()
                value_optim.step()
            #
        except:
            extype,value,tb = sys.exc_info()
            traceback.print_exc()
            pdb.post_mortem(tb)
        loss_list.append(total_loss)
    #
    calculate_advantages(rollouts, value)

    # Learn a policy
    policy_dataset = PolicyDataset(rollouts)
    policy_loader = DataLoader(policy_dataset, batch_size=policy_batch_size, shuffle=True, pin_memory=True)
    for _ in range(policy_epochs):
        # train policy network
        for state, probs, action_index, reward, ret, advantage in policy_loader:
            policy_optim.zero_grad()
            current_batch_size = reward.size()[0]
            advantage = advantage.detach().float()#ret.float()
            p = policy(state.float())
            # print(p.shape, action_index.)
            # print(action_index)
            # print(p[range(current_batch_size, action_index)])
            action_index_list = []
            tmp = action_index
            while tmp >=0:
                action_index_list.append(tmp%24)
                tmp -= tmp%24
                tmp /= 24
            print(action_index_list)

            try:
                ratio = 0
                for index in action_index:
                    ratio += p[range(current_batch_size), index] / probs[range(current_batch_size), index]

            except:
                extype,value,tb = sys.exc_info()
                traceback.print_exc()
                pdb.post_mortem(tb)
            # ratio = p[range(current_batch_size), action_index] / probs[range(current_batch_size), action_index]

            lhs = ratio*advantage
            rhs = torch.clamp(ratio, 1-epsilon, 1+epsilon) * advantage
            loss = -torch.mean(torch.min(lhs, rhs))
            loss.backward()
            policy_optim.step()
        #
    #
    # loop.set_description('standing time: {}'.format(avg_standing_time))
    # if epoch % 1 == 0 and epoch != 0:
    if epoch % 10 == 0:
        torch.save(policy, 'policy-{}'.format(epoch))
        torch.save(value, 'value-{}'.format(epoch))
        np.save('reward_list', reward_list, allow_pickle=False, fix_imports=False)
    #
    # loop.set_description('Reward: {:.4f}'.format(avg_reward.item()))
    # loop.set_description(f'Reward: {avg_reward.item():.4f}')
    # loop.update(1)
    #
#
